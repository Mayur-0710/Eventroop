...
class AttendanceConsumer(AsyncWebsocketConsumer):
    ...
    WebSocket consumer that:
      ...
      4. Sends the saved record + live AttendanceReport summary to the browser.
    ...
    async def push_attendance(self, event):
        ...
        # 3. Push the latest monthly report so the UI stays in sync
    ...
    def get_latest_report(self, period_type: str = "MONTHLY"):
        """
        Return the most recent AttendanceReport row for the given period_type.
        Ordered by -start_date (as per your model Meta).
        """
        report = (
            AttendanceReport.objects
            .filter(user=self.user, period_type=period_type)
            .order_by("-start_date")
            .first()
        )
        if not report:
